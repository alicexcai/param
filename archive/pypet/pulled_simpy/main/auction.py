...
def agent_slot(occupants, a_id, t):
    # Return the slot agent with id a_id occupied in round t, or -1 if a_id wasn't present in round t
    agents = occupants[t]
    if a_id in agents:
...
def sim(config):
    ...
    def total_spent(agent_id, end):
        ...
        # Compute total amount spent by agent_id through (not including) round end.
    ...
    def run_round(top_slot_clicks, t):
        ...
        # Save utility (misnamed as values)
...
def init_agents(conf):
    # Each agent class must be already loaded, and have a constructor that takes an id, a value, and a budget, in that order.
...
def parse_agents(args):
    # Each element is a class name like "Peer", with an optional count appended after a comma.  So either "Peer", or "Peer,3".
    # Returns an array with a list of class names, each repeated the specified number of times.
...
def main(params):
    ...
    # leftover args are class names: e.g. "Truthful BBAgent CleverBidder Fred
    ...
    options = params
    logging.info("options, params", options, params)
    agents_to_run = params.agent_class_names
    logging.info("AGENTS TO RUN:", agents_to_run)
    # agents_to_run = ['Truthful', 'Truthful', 'Truthful']
    ...
    for a in range(n):
        ...
        ### ADDED CODE ###
        agent_stats[agents_to_run[a]] = {'Average spend': 0.01 *total_spent[a]/N, 'Average utility': 0.01 * totals[a]/N}
        ### ADDED CODE ###  
    ...
    sim_stats = {'Average daily revenue': 0.01 * m, 'Stddev daily revenue': 0.01*std, 

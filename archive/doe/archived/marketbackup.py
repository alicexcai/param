...
def sim(params, meta_params):
    ...
    results_full = pd.DataFrame(columns=meta_params.params_const + meta_params.params_tested + meta_params.results_full)
    results_primary = pd.DataFrame(columns=meta_params.results_primary)
    results_full['round_num'] = range(params.num_rounds+1)
    
    print("\n\n========\n\n", results_full, "\n\n========\n\n")

    outcomes = results_full['outcomes'] = params.outcomes
    mechanism = results_full['mechanism'] = params.mechanism 
    liquidity = results_full['liquidity'] = params.liquidity
    num_rounds = results_full['num_rounds'] = params.num_rounds
    
    # variable
    agents_list = results_full['agents_list'] = params.agents_list
